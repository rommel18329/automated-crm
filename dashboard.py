...
with st.expander("Import leads from CSV"):
    uploaded = st.file_uploader("Upload CSV", type=["csv"], key="lead_csv")
    clear_old = st.checkbox("Clear existing leads before import", value=False)
    if st.button("Import Leads", disabled=uploaded is None):
        if uploaded is not None:
            temp_path = Path(".uploaded_leads.csv")
            temp_path.write_bytes(uploaded.getvalue())
            summary = db.import_leads_from_csv(str(temp_path), clear_existing=clear_old)
            if summary["errors"]:
                st.warning(f"Imported {summary['imported']} leads with {len(summary['errors'])} row errors.")
                for err in summary["errors"][:20]:
                    st.write(f"- {err}")
            else:
                st.success(
                    f"Imported {summary['imported']} leads "
